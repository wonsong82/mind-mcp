# mind-mcp Installation Guide — OpenCode (OmO)
...
## Step 4 — Install the Write Hook

The write hook nudges the orchestrator to dispatch mind-curator after each user interaction. It uses a lock file to fire exactly once per turn. OmO reads `.claude/settings.json` via its Claude Code compatibility layer — this single file drives both platforms.
...
## Step 5 — Add Mind Instructions to AGENTS.md

Add the following block to the project's `AGENTS.md` (create the file if it doesn't exist). Place it near the top so it's loaded early in every session.
...
### Mind Curator Dispatch

When dispatching mind-curator, use:

```
task(category="unspecified-low", load_skills=[], run_in_background=false, description="Persist to memory", prompt="Role: mind-curator\nLoad .mind-mcp/install/framework/agent-instructions/mind-curator.md and follow it.\n\n{your dispatch content here}")
```

### Rules

- **"Make a note" / "remember this"** — dispatch mind-curator to persist to memory. Include the user's note in the dispatch prompt.
...
Restart your OpenCode session. On the first message, you should see `mind_boot()` called as the first tool action. If it's not called, check that the AGENTS.md instructions were added correctly.
...
your-project/
├── .claude/
│   └── settings.json              # Write hook (OmO reads via compat layer)
...
└── AGENTS.md                      # Agent instructions (with mind section)
```
